...
const ClineProvider_1 = require("./src/core/webview/ClineProvider");
const port = 3000;
...
app.use((req, res, next) => {
    ...
    if (!validApiKey) {
        validApiKey = process.env.CLINE_API_KEY || null;
    }
...
function getClineProvider() {
    try {
        const provider = ClineProvider_1.ClineProvider.getVisibleInstance();
        if (!provider) {
            throw new Error("Cline provider not available");
        }
        return provider;
    }
    catch (error) {
...
// --- Task Management ---
app.post("/api/tasks", createHandler(async (provider, req) => {
...
app.post("/api/tasks/:taskId/resume", createHandler(async (provider, req) => {
    const { taskId } = req.params;
    const { historyItem } = await provider.getTaskWithId(taskId);
    await provider.initClineWithHistoryItem(historyItem);
    return { status: 200, data: { success: true } };
}));
app.post("/api/tasks/:taskId/cancel", createHandler(async (provider, req) => {
    const { taskId } = req.params;
    await validateTaskId(provider, taskId);
    await provider.cancelTask();
    return { status: 200, data: { success: true } };
}));
app.get("/api/tasks", createHandler(async (provider) => {
    const taskHistory = await provider.getGlobalState("taskHistory") || [];
    return taskHistory;
}));
app.get("/api/tasks/:taskId", createHandler(async (provider, req) => {
    const { taskId } = req.params;
    const { historyItem } = await provider.getTaskWithId(taskId);
    if (!historyItem) {
...
app.delete("/api/tasks/:taskId", createHandler(async (provider, req) => {
    const { taskId } = req.params;
    await provider.deleteTaskWithId(taskId);
    return { status: 200, data: { success: true } };
}));
app.get("/api/tasks/:taskId/export", createHandler(async (provider, req) => {
    const { taskId } = req.params;
    return provider.exportTaskWithId(taskId);
}));
// --- Interaction with Cline ---
app.post("/api/tasks/:taskId/response", createHandler(async (provider, req) => {
    const { taskId } = req.params;
...
// --- Webview Management ---
app.get("/api/state", createHandler(async (provider) => {
    return provider.getStateToPostToWebview();
}));
app.post("/api/webview/message", createHandler(async (provider, req) => {
    await provider.postMessageToWebview(req.body);
    return { status: 200, data: { success: true } };
}));
// --- Settings Management ---
app.put("/api/settings/api", createHandler(async (provider, req) => {
    await provider.updateApiConfiguration(req.body);
    return { status: 200, data: { success: true } };
}));
app.put("/api/settings/customInstructions", createHandler(async (provider, req) => {
    const { instructions } = req.body;
    await provider.updateCustomInstructions(instructions);
    return { status: 200, data: { success: true } };
}));
app.put("/api/settings/autoApproval", createHandler(async (provider, req) => {
    await provider.updateGlobalState("autoApprovalSettings", req.body);
    return { status: 200, data: { success: true } };
}));
app.put("/api/settings/browser", createHandler(async (provider, req) => {
    await provider.updateGlobalState("browserSettings", req.body);
    return { status: 200, data: { success: true } };
}));
app.put("/api/settings/chat", createHandler(async (provider, req) => {
    await provider.updateGlobalState("chatSettings", req.body);
    return { status: 200, data: { success: true } };
}));
app.put("/api/settings/chat/mode", createHandler(async (provider, req) => {
    const { mode } = req.body;
    await provider.togglePlanActModeWithChatSettings({ mode });
    return { status: 200, data: { success: true } };
}));
// --- Authentication ---
app.post("/api/auth/token", createHandler(async (provider, req) => {
    const { token } = req.body;
    await provider.setAuthToken(token);
    return { status: 200, data: { success: true } };
}));
app.post("/api/auth/user", createHandler(async (provider, req) => {
    const { displayName, email, photoURL } = req.body;
    await provider.setUserInfo({ displayName, email, photoURL });
    return { status: 200, data: { success: true } };
}));
app.post("/api/auth/signout", createHandler(async (provider) => {
    await provider.handleSignOut();
    return { status: 200, data: { success: true } };
}));
// --- MCP Management ---
app.get("/api/mcp/marketplace", createHandler(async (provider) => {
    const catalog = await provider.getGlobalState("mcpMarketplaceCatalog");
    return catalog;
}));
app.post("/api/mcp/download", createHandler(async (provider, req) => {
    const { mcpId } = req.body;
    await provider.downloadMcp(mcpId);
    return { status: 200, data: { success: true } };
}));
app.put("/api/mcp/servers/:serverName/toggle", createHandler(async (provider, req) => {
    const { serverName } = req.params;
    const { disabled } = req.body;
    await provider.mcpHub?.toggleServerDisabled(serverName, disabled);
    return { status: 200, data: { success: true } };
}));
app.put("/api/mcp/servers/:serverName/tools/:toolName/toggleAutoApprove", createHandler(async (provider, req) => {
    const { serverName, toolName } = req.params;
    const { autoApprove } = req.body;
    await provider.mcpHub?.toggleToolAutoApprove(serverName, toolName, autoApprove);
    return { status: 200, data: { success: true } };
}));
app.post("/api/mcp/servers/:serverName/restart", createHandler(async (provider, req) => {
    const { serverName } = req.params;
    await provider.mcpHub?.restartConnection(serverName);
    return { status: 200, data: { success: true } };
}));
app.delete("/api/mcp/servers/:serverName", createHandler(async (provider, req) => {
    const { serverName } = req.params;
    await provider.mcpHub?.deleteServer(serverName);
    return { status: 200, data: { success: true } };
}));
// --- Miscellaneous ---
app.post("/api/subscribe", createHandler(async (provider, req) => {
